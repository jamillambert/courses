...
fn read_options(arg: &String) -> Vec<char> {
    // Returns a vector of characters of all implemented options input
...
fn print_help() {
    // Prints the help text and then exits
    ...
    println!(
        "\t\toutput file1.txt and then file2.txt contents with line 
            numbering and"
    );
...
fn main() {
    // Reads in the options, if -h is used the help is printed and the program
    // exits, otherwise each file in the list is passed to cat() to be output
