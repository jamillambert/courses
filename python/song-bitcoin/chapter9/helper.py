...
def bits_to_target(bits):
    ...
    exponent = bits[-1] # last byte is exponent
    coefficient = little_endian_to_int(bits[:3]) # the first three bytes are the coefficient in little endian
    # the formula is:
...
def calculate_new_bits(previous_bits, time_differential):
    ...
    if time_differential > TWO_WEEKS * 4:
        time_differential = TWO_WEEKS * 4 # if the differential > 8 weeks, set to 8 weeks
    elif time_differential < TWO_WEEKS // 4:
        time_differential = TWO_WEEKS // 4 # if the differential < 1/2 week, set to 1/2 week
    new_target = bits_to_target(previous_bits) * time_differential // TWO_WEEKS # new target is last target * differential / 2 weeks
    return target_to_bits(new_target) # convert new target to bits# if the time differential is greater than 8 weeks, set to 8 weeks
