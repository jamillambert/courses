...
class Block:
    ...
    def parse(cls, s):
        ...
        prev_block = s.read(32)[::-1] # prev_block - 32 bytes, little endian (use [::-1] to reverse)
        merkle_root = s.read(32)[::-1] # merkle_root - 32 bytes, little endian (use [::-1] to reverse)
        ...
        return cls(version, prev_block, merkle_root, timestamp, bits, nonce) # initialize class
    ...
    def hash(self):
        ...
        s = self.serialize() # serialize
        sha = hash256(s) # hash256
        return sha[::-1] # reverse
    ...
    def difficulty(self):
        ...
        lowest = 0xffff * 256**(0x1d - 3) # lowest difficulty has bits that equal 0xffff001d
        return lowest / self.target() # note difficulty is (target of lowest difficulty) / (self's target)

    def check_pow(self):
        '''Returns whether this block satisfies proof of work'''
        sha = hash256(self.serialize()) # get the hash256 of the serialization of this block
        sha_num = little_endian_to_int(sha) # interpret this hash as a little-endian number
        return sha_num < self.target() # return whether this integer is less than the target
